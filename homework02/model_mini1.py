...
class MiniNet(pl.LightningModule):
    """Baby Baby ResNet model
    
    This version includes 2 residual layers and 1 fully connected layer.
    
    Input: 3*64*64 image
    Layer 0: 5*5 convolution with 32 channels and stride 2
    ...
    def __init__(
        ...
        self.layer_1 = self._make_layer(
            4, initial_channel, initial_channel * 2
        )  # Returns 32 * 32
        self.layer_2 = self._make_layer(
            4, initial_channel * 2, initial_channel * 4
        )  # Returns 16 * 16
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))

        self.fc1 = nn.Linear(initial_channel * 4, 200)
...
class MiniNetStudent(MiniNet):
    """
    Just like MiniNet, except that at the train step it trains on inputs of the teacher model
    """
